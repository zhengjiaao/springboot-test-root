package com.zja.easyexcel.db.util.id;
...
/**
 * 数据库Id生成策略，生成规则与MongoDB ID生成策略保持一致
 * 由hutools的{@link cn.hutool.core.lang.ObjectId} 改写而来 <br>
 * MongoDB ID生成策略实现<br>
 * ObjectId由以下几部分组成：
 *
 * <pre>
 * 1. Time 时间戳。
 * 2. Machine 所在主机的唯一标识符，一般是机器主机名的散列值。本类使用随机数代替
 * 3. PID 进程ID。确保同一机器中不冲突。本类使用随机数代替
