 ...
 * @Since:
 */
package com.zja.pool.hikaricp.model.dto;
...
public final class PageData<T> implements Serializable {
    ...
    @Deprecated
    public <R extends Serializable> PageData<R> transfer(Function<T, R> mapper) {
        List<R> data = this.getData().stream().map(mapper).collect(Collectors.toList());
        return of(data, this.getIndex(), this.getSize(), this.getCount());
    }
