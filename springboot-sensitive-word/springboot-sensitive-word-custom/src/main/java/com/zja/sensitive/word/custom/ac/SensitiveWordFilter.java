package com.zja.sensitive.word.custom.ac;
...
public class SensitiveWordFilter {
    ...
    public static void main(String[] args) {
        // 定义敏感词列表
        String[] sensitiveWords = {"敏感", "敏感词", "敏感词1", "敏感词2", "敏感词3", "新敏感词", "傻", "傻子", "真傻子"};

        // 创建敏感词过滤器
        SensitiveWordFilter filter = new SensitiveWordFilter(sensitiveWords);

        // 测试文本
        String text = "敏感，敏感词测试文本，这是一个包含敏感词1的测试文本，其中还有敏感词2，这是一个包含新敏感词的测试文本。傻，傻子，真傻子啊";

        // 检测文本中是否包含敏感词
        System.out.println("文本中是否包含敏感词: " + filter.containsSensitiveWord(text));

        // 替换文本中的敏感词
        String replacedText = filter.replaceSensitiveWords(text, '*');
        System.out.println("替换后的文本: " + replacedText); // 替换后的文本: **，***测试文本，这是一个包含****的测试文本，其中还有****，这是一个包含****的测试文本。*，**，***啊
    }
}